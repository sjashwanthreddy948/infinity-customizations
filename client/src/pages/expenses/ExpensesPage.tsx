    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card">
          <p className="text-[10px] uppercase font-bold text-slate-400">Total General Expenses</p>
        ...
        {categoryTotals.slice(0, 3).map((ct: any) => (
          <div key={ct.category} className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card">
            <p className="text-[10px] uppercase font-bold text-slate-400">{ct.category}</p>
            <p className="text-xl font-bold text-[#0B3A82] dark:text-white mt-1">₹{ct.total.toLocaleString('en-IN')}</p>
            <p className="text-[10px] text-slate-400">{ct.count} entries</p>
          </div>
        ))}
      </div>

      {/* Filter & Search Bar */}
      <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card flex flex-col sm:flex-row items-center gap-3">
        ...
        <select
          ...
          className="w-full sm:w-auto px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-blue-900/60 bg-slate-50 dark:bg-[#051E44] text-[#172033] dark:text-white"
        >
          ...
          expenses.map((exp: any) => (
            ...
            >
              ...
              <div>
                ...
                {exp.notes && <p className="text-[11px] text-slate-500 italic mt-0.5">{exp.notes}</p>}
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-600 font-medium">{exp.created_by_name}</span>
                  <span className="text-[10px] text-slate-400 font-mono">({exp.payment_method})</span>
                </div>
                ...
                {expenses.map((exp: any) => (
                  <tr key={exp.id} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="py-3 px-4 font-semibold text-[#172033]">
                      {exp.description}
                      {exp.notes && <p className="text-[10px] text-slate-400 font-normal italic">{exp.notes}</p>}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-slate-600 font-medium">{exp.created_by_name}</span>
                      </div>
                    </td>
                  </tr>
                ))}
