    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card">
          <p className="text-[11px] uppercase font-bold tracking-wider text-slate-400 dark:text-slate-300">Filtered Revenue</p>
          <p className="text-xl font-bold text-[#0B3A82] dark:text-white mt-1">₹{totalRevenue.toLocaleString('en-IN')}</p>
        </div>
        <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card">
          <p className="text-[11px] uppercase font-bold tracking-wider text-slate-400 dark:text-slate-300">Total Order Costs</p>
          <p className="text-xl font-bold text-rose-600 dark:text-rose-400 mt-1">₹{totalCost.toLocaleString('en-IN')}</p>
        </div>
        <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-950/30 dark:to-blue-900/40 border border-[#D4AF37]/40 shadow-card">
          <p className="text-[11px] uppercase font-bold tracking-wider text-amber-800 dark:text-[#F5E7B2]">Net Calculated Profit</p>
          <p className="text-xl font-black text-[#D4AF37] mt-1">₹{totalProfit.toLocaleString('en-IN')}</p>
        </div>
        <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card">
          <p className="text-[11px] uppercase font-bold tracking-wider text-slate-400 dark:text-slate-300">Available Money</p>
      ...
      {/* Filter Bar */}
      <div className="p-4 rounded-2xl bg-white dark:bg-[#082A5E] border border-slate-100 dark:border-blue-900/50 shadow-card flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs rounded-xl border border-slate-200 dark:border-blue-900/60 bg-slate-50 dark:bg-[#051E44] text-[#172033] dark:text-white focus:outline-none focus:ring-2 focus:ring-[#0B3A82]"
          />
        </div>
        ...
        <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 w-full md:w-auto">
          <select
            ...
            className="w-full sm:w-auto px-2.5 sm:px-3 py-2 text-xs rounded-xl border border-slate-200 bg-slate-50 text-[#172033]"
          >
        ...
        {isLoading ? (
          ...
          orders.map((o) => (
            <div
              ...
              className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm active:bg-slate-50 transition-colors space-y-3 cursor-pointer"
            >
              {/* Card Header: Order #, Date, and Status */}
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-mono font-black text-sm text-[#0B3A82]">#{o.order_number}</span>
                  <span className="text-[10px] text-slate-400 block">{o.order_date}</span>
                </div>
              ...
              <div className="border-t border-slate-100 pt-2 flex items-start justify-between gap-2">
                ...
                <div className="text-right">
                  <p className="text-xs font-semibold text-[#0B3A82] flex items-center justify-end gap-1">
                    <span>{o.product_name}</span>
                    <span className="px-1.5 py-0.2 rounded bg-slate-100 text-[10px] font-bold">x{o.quantity}</span>
                  </p>
                  {o.is_tshirt ? (
                  ...
                  {o.has_id_cards === 1 && (
                    <span className="inline-block mt-1 px-2 py-0.5 rounded-full bg-blue-50 text-[#0B3A82] border border-blue-200 text-[9px] font-bold">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="text-[9px] uppercase font-bold text-slate-400 block">Customer Total</span>
                  <span className="text-xs font-black text-slate-900">₹{o.selling_price?.toLocaleString('en-IN')}</span>
                </div>
                <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="text-[9px] uppercase font-bold text-slate-400 block">Total Cost</span>
                  <span className="text-xs font-black text-rose-600">₹{o.total_cost?.toLocaleString('en-IN')}</span>
                </div>
                <div className="p-2 rounded-xl bg-amber-50/60 border border-[#D4AF37]/30">
                  <span className="text-[9px] uppercase font-bold text-amber-800 block">Net Profit</span>
                  <span className="text-xs font-black text-[#D4AF37]">₹{o.profit?.toLocaleString('en-IN')}</span>
                </div>
                <div className="p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="text-[9px] uppercase font-bold text-slate-400 block">Available Cash</span>
                  <span className={`text-xs font-black ${o.available_amount >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
              ...
              <div className="border-t border-slate-100 pt-2 flex items-center justify-between text-[11px] text-slate-500">
                ...
                <span className="text-[#0B3A82] font-bold flex items-center gap-0.5">
                  View Details →
                </span>
              </div>
            </div>
      ...
      <div className="hidden md:block bg-white dark:bg-[#082A5E] rounded-2xl border border-slate-100 dark:border-blue-900/50 shadow-card overflow-hidden">
        {isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#F7F9FC] dark:bg-blue-950/60 text-slate-500 dark:text-slate-300 font-semibold border-b border-slate-100 dark:border-blue-900/40 uppercase tracking-wider text-[10px]">
                <tr>
                ...
                {orders.map((o) => (
                  ...
                  >
                    <td className="py-3 px-4">
                      ...
                      <p className="text-[11px] text-slate-400 dark:text-slate-400">{o.order_date}</p>
                    </td>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-medium text-[#172033] dark:text-slate-200">{o.product_name}</span>
                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-blue-900/60 font-semibold">
                          x{o.quantity}
                        </span>
                      </div>
                      {o.is_tshirt ? (
                        <div className="text-[10px] text-slate-500 mt-0.5 space-y-0.5">
                          ...
                          {o.tshirt_size_breakdown ? (
                            <p className="font-semibold text-[#0B3A82]">
                              Sizes: <span className="font-bold text-slate-800">{o.tshirt_size_breakdown}</span>
                            </p>
                          ) : o.tshirt_size ? (
                            <p>
                              Size: <span className="font-bold text-slate-700">{o.tshirt_size}</span>
                            </p>
                          ) : null}
                          <p className="text-slate-400">
                    ...
                    <td className="py-3 px-4 text-right">
                      ...
                      <p className="text-[10px] text-slate-400">{o.profit_margin}% margin</p>
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-400 hover:text-[#0B3A82] dark:hover:text-[#D4AF37] hover:bg-slate-100 dark:hover:bg-blue-900/50"
                      >
