...
export function calculateOrderPartnerShare(order: {
  ...
  if (isShared) {
    ...
    return {
      isShared: true,
      categoryTag: 'SHARED',
      scopeLabel: 'Shared (T-Shirts, ID Cards, Caps)',
      jashwanthShare: half,
      rajshekarShare: otherHalf,
      jashwanthPercentage: 50,
      rajshekarPercentage: 50,
      explanation: '50/50 partnership profit share applies (T-Shirts, ID Cards & Caps).'
    };
  } else {
    return {
      isShared: false,
      categoryTag: 'SOLE',
      scopeLabel: 'Sole / Jashwanth Only (Bouquets, Frames, Gifts)',
