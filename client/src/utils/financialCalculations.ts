...
export function calculateOrderFinancials(input: FinancialInput): FinancialSummary {
  ...
  const tshirtRapido = Math.max(0, Math.round(Number(input.tshirtRapidoCost) || 0));
  const printRapido = Math.max(0, Math.round(Number(input.printRapidoCost) || 0));
  const explicitRapido = input.rapidoCost !== undefined
    ? Math.max(0, Math.round(Number(input.rapidoCost) || 0))
    : (tshirtRapido + printRapido);
  const rawDelivery = Math.max(0, Math.round(Number(input.deliveryCost) || 0));

  // If rawDelivery is equal to explicitRapido, deliveryCost was passed as the total logistics sum
  let finalDelivery = rawDelivery;
  let finalRapido = explicitRapido;
  if (rawDelivery > 0 && explicitRapido > 0 && rawDelivery === explicitRapido) {
    finalDelivery = 0;
  }
  const totalLogistics = finalRapido + finalDelivery;
  ...
  const totalCost = productCost + printingCost + totalLogistics + otherCost;
